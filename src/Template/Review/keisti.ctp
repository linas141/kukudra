...
?>
<head>
    <title>Atsiliepimo redagavimas » Kukudra</title>
</head>
<div class="review form large-9 medium-8 columns content">
    <?= $this->Form->create($review) ?>
    <fieldset>
        <legend><?= __($review->id_Review .' atsiliepimo redagavimas') ?></legend>
		<?php echo $this->Html->link('&nbsp;&nbsp;'.$this->Html->tag('i', '', array('class' => 'glyphicon glyphicon-fast-backward')).'   Grįžtį į profilį', ['controller'=>'User', 'action' => 'profile'], array('escape' => false));?><br><br>
        <?php
            echo $this->Form->control('Review', array('label'=>'Atsiliepimo turinys', 'type'=>'text', 'id'=>'Review','type'=>'text'));
