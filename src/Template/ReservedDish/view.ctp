<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\ReservedDish $reservedDish
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Reserved Dish'), ['action' => 'edit', $reservedDish->fk_ReservationNumber]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Reserved Dish'), ['action' => 'delete', $reservedDish->fk_ReservationNumber], ['confirm' => __('Are you sure you want to delete # {0}?', $reservedDish->fk_ReservationNumber)]) ?> </li>
        <li><?= $this->Html->link(__('List Reserved Dish'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Reserved Dish'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Dish'), ['controller' => 'Dish', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Dish'), ['controller' => 'Dish', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="reservedDish view large-9 medium-8 columns content">
    <h3><?= h($reservedDish->fk_ReservationNumber) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Dish') ?></th>
            <td><?= $reservedDish->has('dish') ? $this->Html->link($reservedDish->dish->id_Dish, ['controller' => 'Dish', 'action' => 'view', $reservedDish->dish->id_Dish]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Fk ReservationNumber') ?></th>
            <td><?= $this->Number->format($reservedDish->fk_ReservationNumber) ?></td>
        </tr>
    </table>
</div>
